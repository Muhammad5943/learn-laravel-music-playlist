...
//  const [input/state field table (value), dispatcher/ variable] = useState([])
    ...
    const store = async (e) => {
        ...
        // console.log(request);
        try {
            let response = await axios.put(props.endpoint, request) // props is a method which has function to get the properties in JS statement that render in ReactDOM
    ...
    const getLyric = async (e) => {
        let {data} = await axios.get(props.endpoint)
        let responseAlbums = await axios.get(`/albums/get-album-by-${ data.band_id }`)
